...
uint8_t ADS8688_Init(ADS8688 *ads, SPI_HandleTypeDef *spiHandle, GPIO_TypeDef *csPinBank, uint16_t csPin)
{
    ...
    HAL_Delay(10);
    // 设置所有通道范围（第57页）
    // 0x05 -> 输入范围设置为0到2.5倍VREF（对于VREF=5伏，这意味着0-10伏范围）
    // 0x06 -> 输入范围设置为0到1.25倍VREF（对于VREF=5伏，这意味着0-5伏范围）
    ads_data[0] = 0x06;
...
{
    HAL_StatusTypeDef ret;
    uint8_t txbuf[2] = {0x00, cmd}; // [15:9]->地址[6:0], [8]->1, [7:0]->数据[7:0]（stm32使用小端序所以需要反转）
    uint8_t rxbuf[4];
